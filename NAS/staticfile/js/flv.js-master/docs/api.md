...
flv.js.map exports all the interfaces through `flvjs` object which exposed in global context `window`.
...
A global interface which include several static getter/setter to set flv.js.map logcat verbose level.
...
| MEDIA_FORMAT_UNSUPPORTED        | The input MediaDataSource format is not supported by flv.js.map |
